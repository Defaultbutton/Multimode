...
from UGtransformer.utils.gat_utils import load_data,get_kfold_data
...
def get_data(graph):
    Adj_block = get_Adj_matrix(graph)#graph.edge_index.to(device)#
    y_one_hot = F.one_hot(graph.y, num_classes=5).float()
...
model = GAT(nfeat=feature_dim_size, nhid=args.ff_hidden_size,
                        nclass=num_classes, dropout=args.dropout,
                        alpha=0.01,
                        nheads=1).to(device) # nhead is set to 1 as the size of input feature vectors is odd
...
def train():
    ...
    for graph in train_set:
        ...
        optimizer.zero_grad()
        prediction_score = model.forward(node_features, Adj_block)
...
def evaluate():
    ...
    with torch.no_grad():
        ...
        for i, graph in enumerate(test_set):
            ...
            prediction_score = model.forward(node_features, Adj_block).detach()
            prediction_output.append(torch.unsqueeze(prediction_score, 0))
    prediction_output = torch.cat(prediction_output, 0)
    predictions = prediction_output.max(1, keepdim=True)[1]
    #print(predictions)
