...
parser.add_argument("--num_epochs", default=2000, type=int, help="Number of training epochs")
parser.add_argument("--model_name", default='UGformer', help="")
parser.add_argument("--dropout", default=0.5, type=float, help="")
parser.add_argument("--num_hidden_layers", default=3, type=int, help="")
...
train_loader, test_loader = get_kfold_data(5, 1, graphs)
...
def nll_loss(pre, targets):
    logsoftmax = nn.LogSoftmax(dim=1)
    loss_fn = nn.NLLLoss()
    loss = loss_fn(logsoftmax(pre), targets)
    return loss
...
def train():
    ...
    for i, graph in enumerate(train_loader):
        ...
        graph_label = F.one_hot(graph.y, num_classes=5).float().to(device)
         # one graph per step. should modify to use "padding" (for node_features and Adj_block) within a batch size???
        optimizer.zero_grad()
        prediction_score = model.forward(graph)
        prediction_output.append(torch.unsqueeze(prediction_score, 0))
        #loss = cross_entropy(prediction_score, graph_label)
        loss = criterion(prediction_score, graph_label)
        #use NLL_LOSS
        #loss = nll_loss(prediction_score, labels[0])
    ...
    predictions = prediction_output.max(2, keepdim=True)[1]
    correct = predictions.eq(labels.view_as(predictions)).sum().cpu().item()
    acc_train = correct / float(len(train_loader.dataset))
...
def evaluate():
    ...
    with torch.no_grad():
        ...
        for i, graph in enumerate(test_loader):
            ...
            #    continue
            labels.append(graph.y)
            prediction_score = model.forward(graph).detach()
            prediction_output.append(torch.unsqueeze(prediction_score, 0))
    prediction_output = torch.cat(prediction_output, 0)
    predictions = prediction_output.max(2, keepdim=True)[1]
    labels = torch.cat(labels, dim=0).to(device)
