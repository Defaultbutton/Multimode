...
class FullyConnectedGT_UGformerV2(nn.Module):
    ...
    def __init__(self, feature_dim_size, ff_hidden_size, num_classes,
        ...
        for _layer in range(self.num_GNN_layers):
            #encoder_layers = TransformerEncoderLayer(d_model=512, nhead=self.nhead, dim_feedforward=self.ff_hidden_size, dropout=0.5) # Default batch_first=False (seq, batch, feature)
            #self.ugformer_layers.append(TransformerEncoder(encoder_layers, self.num_self_att_layers))
            if _layer == 0 :
                self.lst_gnn.append(GraphConvolution(self.feature_dim_size, 64, act=torch.tanh))
            if _layer == 1:
                self.lst_gnn.append(GraphConvolution(64, 128, act=torch.tanh))
            if _layer == 2:
                self.lst_gnn.append(GraphConvolution(128, 64, act=torch.tanh))
        # Linear function
        ...
        for i in range(self.num_GNN_layers):
            #self.predictions.append(nn.Linear(512, self.num_classes))
            #self.dropouts.append(nn.Dropout(dropout))
            if i == 0:
                self.lins.append(nn.Linear(self.feature_dim_size,128))
            elif i == 1:
                self.lins.append(nn.Linear(128,512))
            elif i == 2:
                self.lins.append(nn.Linear(512,128))
        self.prediction = nn.Linear(64, self.num_classes)
    ...
    def forward(self, graph_batch):
        ...
        for layer_idx in range(self.num_GNN_layers):
            ...
            #input_Tr = self.ugformer_layers[layer_idx](input_Tr) #[batch_size,seq_length, d_model] for pytorch transformer
            # take a sum over neighbors followed by a linear transformation and an activation function --> similar to GCN
            input_Tr = self.lst_gnn[layer_idx](input_Tr, adj)
...
class GraphConvolution(torch.nn.Module):
    ...
    def forward(self, input, adj, padding_mask = None):
